...
function App() {
  ...
  const currentConfigOrDefault = currentConfig ?? defaultConfig;

  const startChat = useCallback(
    async (message: string) => {
      if (!currentConfigOrDefault) return;
      const chat = await createChat(
        message,
        [{ type: "human", content: message }],
        currentConfigOrDefault
      );
      return startStream({ messages: chat.messages }, chat.config.config);
    },
    [createChat, startStream, currentConfigOrDefault]
  );
